...
最小化以下加权和（内部缩放因子=2，将浮点数转为整数运算）：

| 惩罚项 | 内部权重 | 用户可配置 | 说明 |
|-------|---------|-----------|------|
| 过量生产惩罚 | 1 | 否 | 防止无限过量生产 |
| 加班班次惩罚 | 16 | 是（默认50） | 每次加班班次，内部×2÷6归一化 |
| 加班天数惩罚 | 16 | 是（默认50） | 每个有加班的天数，内部×2÷6归一化 |
| 占用休息日惩罚 | 100 | 是（默认50） | 每个被安排生产的休息日，内部×2放大 |
| 产量平滑惩罚 | 5 | 否 | 相邻两天产量差的绝对值之和 |
| 长连续工作惩罚 | 60 | 否 | 连续工作窗口数，引导拆分长连续工作段 |
